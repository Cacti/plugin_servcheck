...
function proxy_form_actions() {
	...
	foreach ($_POST as $var => $val) {
		if (preg_match('/^chk_([0-9]+)$/', $var, $matches)) {
			...
			$proxy_list .= '<li>' . db_fetch_cell_prepared('SELECT name FROM plugin_servcheck_proxies WHERE id = ?', [$matches[1]]) . '</li>';
...
function proxy_edit() {
	...
	draw_edit_form(
		array(
			...
			'fields' => inject_form_variables($servcheck_proxy_fields, (isset($proxy) ? $proxy : []))
		)
	);
