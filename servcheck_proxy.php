			...
			if (get_filter_request_var('drp_action') == 1) {
				if (cacti_sizeof($selected_items)) {
					foreach ($selected_items as $item) {
						db_execute_prepared('DELETE FROM plugin_servcheck_proxy WHERE id = ?', [$item]);
						db_execute_prepared('UPDATE plugin_servcheck_test SET proxy_id = 0 WHERE proxy_id = ?', [$item]);
					}
				}
			}
		}
